          ...
          return (
            <FavesContext.Consumer>
              {values => {
                ...
                const filteredSessionIds = data.allSessions.filter(session => allFaves.includes(session.id))
                console.log("NEWLIST",filteredSessionIds )
                console.log("My GOODISES", allFaves);
                return <Faves data={formatSessionData(filteredSessionIds)} navigation={this.props.navigation} context={values}/>;
              }}
            </FavesContext.Consumer>
          );
        }}
